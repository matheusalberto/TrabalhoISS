...
public class ProdutoController {
    ...
    public double adicionarCompostoManipulado(JFrame tela, int linhaSelecionada, JTable tabelaCompostos, double total, JLabel txtTotal, List<Produto> listaCompostos, String strquantidade, JComboBox cbUnidadeDesejada) {
        ...
        if (produto.getUnidade().equals("Kg")) {
            ...
            if (unidadeDesejada.equals("Kg")) {
                ...
                atualizarEstoque(aux, produto, tela, total, produtoDao, tabelaCompostos, linhaSelecionada, listaCompostos);
    ...
    public double removerCompostos(JFrame tela, JTable tabelaManipulacao, int linhaSelecionada, double total, JLabel txtPreco, List<Produto> listaCompostos) {
        ...
        if (produto.getUnidadeVenda().equals("g")) {
            ...
            aux = produto.getQuantidadeSelecionada() * produto.getPrecoVenda();
            total = total - aux;
            txtPreco.setText(nf.format(total));

        } else if (produto.getUnidadeVenda().equals("mg")) {

            aux = (produto.getQuantidadeSelecionada() * 1000) * produto.getPrecoVenda();
            total = total - aux;
            txtPreco.setText(nf.format(total));
        ...
        if (produto.getUnidade().equals("Kg")) {
            ...
            aux = produto.getQuantidadeEstoque() + produto.getQuantidadeSelecionada() * 1000;
            produto.setQuantidadeEstoque(aux);
            produtoDao.atualizar(produto);

        } else if (produto.getUnidade().equals("g")) {

            aux = produto.getQuantidadeEstoque() + produto.getQuantidadeSelecionada();
            produto.setQuantidadeEstoque(aux);
            produtoDao.atualizar(produto);
    ...
    public void cancelarManipulacao(List<Produto> listaCompostos) {

        double aux = 0;

        for (Produto produto : listaCompostos) {
            ...
            new ProdutoDao().atualizarQuantidadeEstoqueManip(produto.getId(), aux);

        }
    }

    /*public Produto finalizarManipulacao(JFrame tela, List<Produto> listaCompostos, double total, String nome) {

        Produto produto = new Produto();
        produto.setDescricao(nome);

        for (Produto listaComposto : listaCompostos) {
            produto.compostos.add(produto);
        }
